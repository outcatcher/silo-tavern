...
void main() {
  group('Login Error Message Tests:', () {
    test('Returns default message for null input', () {
      ...
      expect(result, 'Authentication failed. Please check your credentials and try again.');
    });
    ...
    test('Returns invalid credentials message for 401 errors', () {
      ...
      expect(result, 'Invalid username or password. Please check your credentials and try again.');
    });
    ...
    test('Returns invalid credentials message for invalid credentials', () {
      final result = getFriendlyAuthErrorMessage('invalid credentials provided');
      expect(result, 'Invalid username or password. Please check your credentials and try again.');
    });
    ...
    test('Returns network error message for SocketException', () {
      final result = getFriendlyAuthErrorMessage('SocketException: Connection failed');
      expect(result, 'Unable to connect to the server. Please check your network connection and try again.');
    });
    ...
    test('Returns timeout message for timeout errors', () {
      ...
      expect(result, 'Connection timed out. The server may be busy or unreachable. Please try again.');
    });
    ...
    test('Returns certificate error message for CERTIFICATE_VERIFY_FAILED', () {
      ...
      expect(result, 'Security certificate verification failed. Please check that the server\'s SSL/TLS certificate is valid.');
    });

    test('Returns certificate error message for HandshakeException', () {
      final result = getFriendlyAuthErrorMessage('HandshakeException: TLS handshake failed');
      expect(result, 'Security certificate verification failed. Please check that the server\'s SSL/TLS certificate is valid.');
    });
