...
class NetworkUtils {
  ...
  static bool isServerConfigurationAllowed(Server server) {
    final isHttp = server.address.startsWith('http://');
    ...
    // HTTPS servers are always allowed
    if (isHttps) {
      return true;
    }
    
    // HTTP servers with authentication are allowed
    if (isHttp && hasAuthentication) {
      return true;
    }
...
class CIDRBlock {
  ...
  CIDRBlock(String networkAddress, this.prefixLength)
      : network = InternetAddress(networkAddress),
        ...
        maskedNetwork = _applyMask(InternetAddress(networkAddress), _createMask(networkAddress, prefixLength));
